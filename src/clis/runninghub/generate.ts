...
cli({
  ...
  func: async (_page, args, debug) => {
    const imageUrls: string[] = args.images ? String(args.images).split(',').map((s: string) => s.trim()).filter(Boolean) : [];
    const videoUrls: string[] = args.videos ? String(args.videos).split(',').map((s: string) => s.trim()).filter(Boolean) : [];
