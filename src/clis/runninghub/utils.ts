...
export function getApiKey(): string {
  ...
  if (!key) {
    throw new CliError(
      'AUTH_ERROR',
      'RUNNINGHUB_API_KEY environment variable is required',
      'Get your API key from https://www.runninghub.cn and set: export RUNNINGHUB_API_KEY=xxx',
    );
  }
  return key;
}

async function runninghubRequest(path: string, body: Record<string, unknown>): Promise<any> {
  ...
  if (!resp.ok) {
    ...
    throw new CliError(
      'FETCH_ERROR',
      `RunningHub API HTTP ${resp.status}: ${text}`,
  ...
  if (data.errorCode && data.errorCode !== 0) {
    throw new CliError(
      'API_ERROR',
      `RunningHub error ${data.errorCode}: ${data.errorMessage ?? 'Unknown error'}`,
      'Check the RunningHub API documentation for error details',
    );
  }
  return data;
}
...
export async function submitTask(body: Record<string, unknown>): Promise<TaskResult> {
...
export async function pollTask(taskId: string, debug: boolean = false): Promise<TaskResult> {
  ...
  while (Date.now() - start < MAX_POLL_MS) {
    ...
    if (result.status === 'SUCCEEDED' || result.status === 'SUCCESS' || result.status === 'FAILED') return result;
