...
export async function navigateToImagine(page) {
...
export async function checkLogin(page) {
  ...
  `);
    if (!state?.loggedIn) {
        throw new AuthRequiredError('www.midjourney.com', 'Please log in to MidJourney in Chrome first. Reason: ' + (state?.reason || 'unknown'));
    }
}
/**
...
export async function fillPrompt(page, prompt) {
  ...
  `);
    if (err) {
        throw new SelectorError('prompt editor', err);
    }
}
/**
...
export async function clickGenerate(page) {
  ...
  `);
    if (result !== 'ok') {
        throw new SelectorError('generate button', result);
    }
}
/**
 ...
 */
export function buildMjPrompt(prompt, opts) {
    let full = prompt;
    if (opts.ar)
        full += ` --ar ${opts.ar}`;
    if (opts.niji) {
    ...
    if (opts.stylize !== undefined && opts.stylize !== 100)
        full += ` --stylize ${opts.stylize}`;
    if (opts.quality && opts.quality !== '1')
        full += ` --quality ${opts.quality}`;
    return full;
}
/**
 * Snapshot all CDN image URLs currently on the page.
 */
export async function snapshotImageUrls(page) {
 ...
 */
export async function waitForNewImages(page, waitSec, existingUrls) {
    ...
    `);
        ...
        const hasUpscale = await page.evaluate(`
      (() => {
 ...
 */
export async function getHistoryList(page, limit) {
    // Try API first
